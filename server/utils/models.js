...
function normalizeModelIdForProvider(modelId, providerId) {
  ...
  const providerPrefix =
    PROVIDER_DEFAULT_GATEWAY_PREFIX[providerId] || providerId;
...
function parseLine(line, providerId) {
  ...
  const displayName = explicitName || prettifyName(normalizedModelId);
  const isCloud = /\(Cloud\)/i.test(displayName);
...
export async function getModelsByCategory(
  ...
  return allModels.filter((model) => {
    ...
    const gatewayPrefixMatch =
      !allowedPrefixes ||
      (modelGatewayPrefix && allowedPrefixes.has(modelGatewayPrefix));
    const modelProviderMatch =
      modelProviderFilter === "all" ||
      model.modelProvider === modelProviderFilter;
    return (
      categoryMatch && providerMatch && gatewayPrefixMatch && modelProviderMatch
    );
  });
}

export async function getAutoModelForProvider(
  config,
  providerId,
  category = "chat",
) {
